...
public class TuringMachine : MonoBehaviour {
	...
	void Awake()
	{
		...
		Debug.LogFormat("[Turing Machine #{0}] Solution: {1}{2}{3}", moduleId, solution[0], solution[1], solution[2]);

		foreach (Clue clue in clues)
		{
			Debug.LogFormat("[Turing Machine #{0}] Clue {1}", moduleId, clue.toString());
			Debug.LogFormat("[Turing Machine #{0}] Verifier {1}", moduleId, clue.getSolution());
		}
	...
	void pressedVerifier(char ID)
	{
		...
		if (posIndex > 2)
		{
			...
			if(result)
			{
				...
				display();
				pressedClear();
				Debug.LogFormat("[Turing Machine #{0}] Test #{1}: {2}{3}{4} {5} {6}", moduleId, results.Count, digits[0], digits[1], digits[2], ID, s);
			}
			else
			{
				Debug.LogFormat("[Turing Machine #{0}] User tried to test {1}{2}{3} but it hasn't reset yet", moduleId, digits[0], digits[1], digits[2]);
		...
		else
		{
			...
			else
			{
				...
				Material[] mats = clues[index].getClueMat();
				for (int i = 0; i < mats.Length; i++)
					mainScreen[i].material = mats[i];
				verifierText[index].color = Color.green;
			}
		}
	}
	...
	void pressedSubmit()
	{
		...
		if(flag)
		{
			Debug.LogFormat("[Turing Machine #{0}] User is submitting {1}{2}{3}", moduleId, numberText[0].text, numberText[1].text, numberText[2].text);
...
#pragma warning disable 414
	private readonly string TwitchHelpMessage = @"!{0} press|p [buttons] will press all the buttons within the buttons attribute. Button list: A, B, C, D, E, F, 1, 2, 3, 4, 5, Clear/X, Left/L, Right/R, Submit/Sub.";
